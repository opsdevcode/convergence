...
- Do **not** add agent or tool attribution trailers (for example
  `Co-authored-by:` lines naming an AI assistant, or "generated by"
  notices) to commits or pull request descriptions. This work is authored
  by its human contributors.
- A local `commit-msg` hook strips known agent-attribution trailers as a
  backstop. It is convenience, not policy: keep the trailers out in the
  first place, and do not rely on the hook in environments that do not run
  it.
